...
type IngenialistsAPIV1 struct{}

func (v1 *IngenialistsAPIV1) Run() {
	godotenv.Load(".env")
	db := v1.SetupDatabase()
	r := v1.SetupServer(db)
...
func (*IngenialistsAPIV1) SetupServer(db *gorm.DB) *gin.Engine {
	r := gin.Default()
...
func (*IngenialistsAPIV1) SetupDatabase() *gorm.DB {
	// Initialize database
	db, err := models.DB()
	if err != nil {
